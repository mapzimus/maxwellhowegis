"""
Aggregate the FIFTH municipal ACS bundle (data/ma_muni_acs_extra4.json) up to
the 274 K-12 academic districts the atlas renders in district mode, and write the
result to ``data/ma_district_acs_extra4.json`` keyed by DIST_CODE.
...
/ ``aggregate_acs_extra3_to_districts.py`` and reuses the exact same
...
aggregation. It only handles the NEW columns from ma_muni_acs_extra4.json:

  acs_has_computer_pct        -> population-weighted mean
  acs_limited_english_hh_pct  -> population-weighted mean
  acs_median_earnings         -> population-weighted mean (raw dollars, rounded)
  acs_renter_cost_burden_pct  -> population-weighted mean
  acs_labor_force_pct         -> population-weighted mean
...
as in aggregate_acs_extra3_to_districts.py. The extra4 file intentionally carries
...
denominator is proportional to total population). acs_median_earnings is a
median, so a population-weighted mean of member-town medians approximates the
district-wide typical earnings, not a true pooled median — but it matches the
...
Run from repo root::
    ...
    python scripts/aggregate_acs_extra4_to_districts.py
"""
...
MUNI_ACS_EXTRA4_PATH = REPO_ROOT / "data" / "ma_muni_acs_extra4.json"
OUT_PATH = REPO_ROOT / "data" / "ma_district_acs_extra4.json"

# Columns to population-weight.
MEAN_COLS = (
    "acs_has_computer_pct",
    "acs_limited_english_hh_pct",
    "acs_median_earnings",
    "acs_renter_cost_burden_pct",
    "acs_labor_force_pct",
)

# Round to a column-appropriate number of decimals on output so re-running is
# deterministic and the JSON diff doesn't churn on FP noise. Earnings is whole
# dollars (0 decimals); the rate columns are 0–1 fractions (6 decimals).
COL_ROUND = {
    "acs_has_computer_pct": 6,
    "acs_limited_english_hh_pct": 6,
    "acs_median_earnings": 0,
    "acs_renter_cost_burden_pct": 6,
    "acs_labor_force_pct": 6,
}
...
def main() -> int:
    ...
    muni_acs_extra4 = json.loads(MUNI_ACS_EXTRA4_PATH.read_text(encoding="utf-8"))
    ...
    for f in munis["features"]:
        ...
        muni_recs[tid] = {
            ...
            "extra": muni_acs_extra4.get(tid, {}),   # the new metrics
        }
