"""
Aggregate the NINTH municipal ACS bundle (data/ma_muni_acs_extra8.json) up to
the 274 K-12 academic districts the atlas renders in district mode, and write the
result to ``data/ma_district_acs_extra8.json`` keyed by DIST_CODE.

This is a companion to ``aggregate_acs_to_districts.py`` …
``aggregate_acs_extra7_to_districts.py`` and reuses the exact same
...
aggregation. It only handles the NEW columns from ma_muni_acs_extra8.json:

  acs_mgmt_occ_pct          -> population-weighted mean (0-1 fraction)
  acs_median_rooms          -> population-weighted mean (raw room count, 1 decimal)
  acs_moved_diff_state_pct  -> population-weighted mean (0-1 fraction)
  acs_govt_worker_pct       -> population-weighted mean (0-1 fraction)
  acs_born_in_state_pct     -> population-weighted mean (0-1 fraction)
...
as in aggregate_acs_extra7_to_districts.py. The extra8 file intentionally carries
no population column of its own.

Caveat: the columns are population-weighted means of member-town values, a sound
approximation of the district-wide value (exact only when each town's denominator
is proportional to total population). For occupation- and worker-based shares
(mgmt occ, govt worker) the proportionality is approximate, and acs_median_rooms
is a weighted mean of town medians (an approximation of the true pooled median).

Run from repo root::

    python scripts/aggregate_acs_extra8_to_districts.py
"""
...
MUNI_ACS_EXTRA8_PATH = REPO_ROOT / "data" / "ma_muni_acs_extra8.json"
OUT_PATH = REPO_ROOT / "data" / "ma_district_acs_extra8.json"

# Columns to population-weight.
MEAN_COLS = (
    "acs_mgmt_occ_pct",
    "acs_median_rooms",
    "acs_moved_diff_state_pct",
    "acs_govt_worker_pct",
    "acs_born_in_state_pct",
)

# Round to a column-appropriate number of decimals on output so re-running is
# deterministic and the JSON diff doesn't churn on FP noise. The four rate columns
# are 0–1 fractions (6 decimals); median rooms is a raw count (1 decimal).
COL_ROUND = {
    "acs_mgmt_occ_pct": 6,
    "acs_median_rooms": 1,
    "acs_moved_diff_state_pct": 6,
    "acs_govt_worker_pct": 6,
    "acs_born_in_state_pct": 6,
}
...
def main() -> int:
    ...
    muni_acs_extra8 = json.loads(MUNI_ACS_EXTRA8_PATH.read_text(encoding="utf-8"))
    ...
    for f in munis["features"]:
        ...
        muni_recs[tid] = {
            ...
            "extra": muni_acs_extra8.get(tid, {}),   # the new metrics
        }
