"""
Aggregate the SEVENTH municipal ACS bundle (data/ma_muni_acs_extra6.json) up to
the 274 K-12 academic districts the atlas renders in district mode, and write the
result to ``data/ma_district_acs_extra6.json`` keyed by DIST_CODE.

This is a companion to ``aggregate_acs_to_districts.py`` …
``aggregate_acs_extra5_to_districts.py`` and reuses the exact same
...
aggregation. It only handles the NEW columns from ma_muni_acs_extra6.json:

  acs_per_capita_income    -> population-weighted mean (raw dollars)
  acs_poverty_pct          -> population-weighted mean
  acs_drove_alone_pct      -> population-weighted mean
  acs_work_from_home_pct   -> population-weighted mean
  acs_crowded_housing_pct  -> population-weighted mean
...
as in aggregate_acs_extra5_to_districts.py. The extra6 file intentionally carries
...
denominator is proportional to total population). acs_per_capita_income is a
per-person dollar figure; population-weighting it approximates the district-wide
per-capita income.

Run from repo root::

    python scripts/aggregate_acs_extra6_to_districts.py
"""
...
MUNI_ACS_EXTRA6_PATH = REPO_ROOT / "data" / "ma_muni_acs_extra6.json"
OUT_PATH = REPO_ROOT / "data" / "ma_district_acs_extra6.json"
...
# 0–1 fractions (6 decimals); per-capita income is raw dollars (0 decimals -> int).
COL_ROUND = {
    "acs_per_capita_income": 0,
    "acs_poverty_pct": 6,
    "acs_drove_alone_pct": 6,
    "acs_work_from_home_pct": 6,
    "acs_crowded_housing_pct": 6,
}
...
def main() -> int:
    ...
    muni_acs_extra6 = json.loads(MUNI_ACS_EXTRA6_PATH.read_text(encoding="utf-8"))
    ...
    for f in munis["features"]:
        ...
        muni_recs[tid] = {
            ...
            "extra": muni_acs_extra6.get(tid, {}),   # the new metrics
        }
