"""
Aggregate the SIXTH municipal ACS bundle (data/ma_muni_acs_extra5.json) up to
the 274 K-12 academic districts the atlas renders in district mode, and write the
result to ``data/ma_district_acs_extra5.json`` keyed by DIST_CODE.

This is a companion to ``aggregate_acs_to_districts.py`` /
``aggregate_acs_extra_to_districts.py`` / ``aggregate_acs_extra2_to_districts.py``
/ ``aggregate_acs_extra3_to_districts.py`` / ``aggregate_acs_extra4_to_districts.py``
and reuses the exact same muni->district join strategy (direct DIST_CODE,
DIST_NAME token parse, and the manual regional-district member table) plus the
same population-weighted mean aggregation. It only handles the NEW columns from
ma_muni_acs_extra5.json:

  acs_grad_degree_pct      -> population-weighted mean
  acs_gini                 -> population-weighted mean (raw 0-1 index)
  acs_pre1960_housing_pct  -> population-weighted mean
  acs_multiunit_pct        -> population-weighted mean
  acs_avg_hh_size          -> population-weighted mean (raw persons)
...
as in aggregate_acs_extra4_to_districts.py. The extra5 file intentionally carries
...
denominator is proportional to total population). acs_gini is itself a derived
index, so a population-weighted mean of member-town Gini indices approximates the
district-wide inequality, not a true pooled Gini — but it matches the documented
approach used in the companion aggregators for index/median metrics.
acs_avg_hh_size is a per-household mean; population-weighting it approximates the
district-wide average household size.

Run from repo root::

    python scripts/aggregate_acs_extra5_to_districts.py
"""
...
MUNI_ACS_EXTRA5_PATH = REPO_ROOT / "data" / "ma_muni_acs_extra5.json"
OUT_PATH = REPO_ROOT / "data" / "ma_district_acs_extra5.json"

# Columns to population-weight.
MEAN_COLS = (
    "acs_grad_degree_pct",
    "acs_gini",
    "acs_pre1960_housing_pct",
    "acs_multiunit_pct",
    "acs_avg_hh_size",
)

# Round to a column-appropriate number of decimals on output so re-running is
# deterministic and the JSON diff doesn't churn on FP noise. The rate columns are
# 0–1 fractions (6 decimals); the Gini index is a 0-1 index (4 decimals);
# average household size is persons (2 decimals).
COL_ROUND = {
    "acs_grad_degree_pct": 6,
    "acs_gini": 4,
    "acs_pre1960_housing_pct": 6,
    "acs_multiunit_pct": 6,
    "acs_avg_hh_size": 2,
}
...
def main() -> int:
    ...
    muni_acs_extra5 = json.loads(MUNI_ACS_EXTRA5_PATH.read_text(encoding="utf-8"))
    ...
    for f in munis["features"]:
        ...
        muni_recs[tid] = {
            ...
            "extra": muni_acs_extra5.get(tid, {}),   # the new metrics
        }
