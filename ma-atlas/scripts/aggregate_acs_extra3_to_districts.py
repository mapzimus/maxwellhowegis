"""
Aggregate the FOURTH municipal ACS bundle (data/ma_muni_acs_extra3.json) up to
the 274 K-12 academic districts the atlas renders in district mode, and write the
result to ``data/ma_district_acs_extra3.json`` keyed by DIST_CODE.
...
ma_muni_acs_extra3.json:
  ...
  acs_median_gross_rent    -> population-weighted mean (raw dollars, rounded)
  acs_single_parent_pct    -> population-weighted mean
  acs_snap_pct             -> population-weighted mean
  acs_veteran_pct          -> population-weighted mean
  acs_moved_last_year_pct  -> population-weighted mean
...
as in aggregate_acs_extra2_to_districts.py. The extra3 file intentionally carries
no population column of its own.

Caveat: the four rate columns are population-weighted means of member-town rates,
a sound approximation of the district-wide rate (exact only when each town's
denominator is proportional to total population). acs_median_gross_rent is a
median, so a population-weighted mean of member-town medians is an approximation
of the district-wide typical rent, not a true pooled median — but it matches the
documented approach used in the companion aggregators for dollar metrics (e.g.
median household income / median home value).

Run from repo root::

    python scripts/aggregate_acs_extra3_to_districts.py
"""
...
MUNI_ACS_EXTRA3_PATH = REPO_ROOT / "data" / "ma_muni_acs_extra3.json"
OUT_PATH = REPO_ROOT / "data" / "ma_district_acs_extra3.json"

# Columns to population-weight.
MEAN_COLS = (
    "acs_median_gross_rent",
    "acs_single_parent_pct",
    "acs_snap_pct",
    "acs_veteran_pct",
    "acs_moved_last_year_pct",
)

# Round to a column-appropriate number of decimals on output so re-running is
# deterministic and the JSON diff doesn't churn on FP noise. Rent is whole
# dollars (0 decimals); the rate columns are 0–1 fractions (6 decimals).
COL_ROUND = {
    "acs_median_gross_rent": 0,
    "acs_single_parent_pct": 6,
    "acs_snap_pct": 6,
    "acs_veteran_pct": 6,
    "acs_moved_last_year_pct": 6,
}
...
def main() -> int:
    ...
    muni_acs_extra3 = json.loads(MUNI_ACS_EXTRA3_PATH.read_text(encoding="utf-8"))
    ...
    for f in munis["features"]:
        ...
        muni_recs[tid] = {
            ...
            "extra": muni_acs_extra3.get(tid, {}),   # the new metrics
        }
