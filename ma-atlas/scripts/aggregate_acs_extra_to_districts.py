"""
Aggregate the EXTRA municipal ACS bundle (data/ma_muni_acs_extra.json) up to the
274 K-12 academic districts the atlas renders in district mode, and write the
result to ``data/ma_district_acs_extra.json`` keyed by DIST_CODE.

This is a companion to ``aggregate_acs_to_districts.py`` and reuses the exact
same muni->district join strategy (direct DIST_CODE, DIST_NAME token parse, and
the manual regional-district member table) plus the same population-weighted
mean aggregation. It only handles the NEW columns from ma_muni_acs_extra.json:

  acs_median_commute_min   -> population-weighted mean
  acs_broadband_pct        -> population-weighted mean
  acs_median_age           -> population-weighted mean
  acs_owner_occupied_pct   -> population-weighted mean
  acs_median_home_value    -> population-weighted mean
...
as in aggregate_acs_to_districts.py. The extra file intentionally carries no
population column of its own.

Caveat: median home value / median commute weighted across member towns is an
approximation (a true median-of-medians needs microdata), matching the documented
approach used for median household income in the basics aggregator.

Run from repo root::

    python scripts/aggregate_acs_extra_to_districts.py
"""
...
MUNI_ACS_EXTRA_PATH = REPO_ROOT / "data" / "ma_muni_acs_extra.json"
OUT_PATH = REPO_ROOT / "data" / "ma_district_acs_extra.json"

# Columns to population-weight. (percentages and raw numbers alike are averaged
# as weighted means; the format/units are handled by the atlas's METRICS entry.)
MEAN_COLS = (
    "acs_median_commute_min",
    "acs_broadband_pct",
    "acs_median_age",
...
# deterministic and the JSON diff doesn't churn on FP noise.
COL_ROUND = {
    "acs_median_commute_min": 2,
    "acs_broadband_pct": 6,
    "acs_median_age": 2,
    "acs_owner_occupied_pct": 6,
    "acs_median_home_value": 0,   # whole dollars
}

# Manual member-town lookup for regional districts whose DIST_NAME is geographic.
# Copied verbatim from aggregate_acs_to_districts.py — keep the two in sync.
...
def main() -> int:
    ...
    muni_acs_extra = json.loads(MUNI_ACS_EXTRA_PATH.read_text(encoding="utf-8"))
    ...
    for f in munis["features"]:
        ...
        muni_recs[tid] = {
            ...
            "extra": muni_acs_extra.get(tid, {}),  # the new metrics
        }
    ...
    def aggregate(town_ids: list[str]) -> dict:
        ...
        agg: dict[str, float | None] = {"_member_count": len(rows)}
        ...
        for col in MEAN_COLS:
            ...
            mean = ws / wt
            nd = COL_ROUND[col]
            agg[col] = round(mean, nd) if nd > 0 else round(mean)
