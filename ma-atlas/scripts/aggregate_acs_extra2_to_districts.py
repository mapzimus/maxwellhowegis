"""
Aggregate the THIRD municipal ACS bundle (data/ma_muni_acs_extra2.json) up to the
274 K-12 academic districts the atlas renders in district mode, and write the
result to ``data/ma_district_acs_extra2.json`` keyed by DIST_CODE.

This is a companion to ``aggregate_acs_to_districts.py`` /
``aggregate_acs_extra_to_districts.py`` and reuses the exact same muni->district
join strategy (direct DIST_CODE, DIST_NAME token parse, and the manual
regional-district member table) plus the same population-weighted mean
aggregation. It only handles the NEW columns from ma_muni_acs_extra2.json:

  acs_unemployment_pct  -> population-weighted mean
  acs_uninsured_pct     -> population-weighted mean
  acs_disability_pct    -> population-weighted mean
  acs_under18_pct       -> population-weighted mean
  acs_no_vehicle_pct    -> population-weighted mean
...
as in aggregate_acs_extra_to_districts.py. The extra2 file intentionally carries
no population column of its own.

Caveat: these are all rate metrics, so a population-weighted mean of member-town
rates is a sound approximation of the district-wide rate (it equals the true
pooled rate only if each town's denominator is proportional to total population;
in practice it's a close approximation, matching the documented approach used in
the companion aggregators).

Run from repo root::

    python scripts/aggregate_acs_extra2_to_districts.py
"""
...
MUNI_ACS_EXTRA2_PATH = REPO_ROOT / "data" / "ma_muni_acs_extra2.json"
OUT_PATH = REPO_ROOT / "data" / "ma_district_acs_extra2.json"

# Columns to population-weight (all 0–1 rate fractions).
MEAN_COLS = (
    "acs_unemployment_pct",
    "acs_uninsured_pct",
    "acs_disability_pct",
    "acs_under18_pct",
    "acs_no_vehicle_pct",
...
COL_ROUND = {
    "acs_unemployment_pct": 6,
    "acs_uninsured_pct": 6,
    "acs_disability_pct": 6,
    "acs_under18_pct": 6,
    "acs_no_vehicle_pct": 6,
}
...
def main() -> int:
    ...
    muni_acs_extra2 = json.loads(MUNI_ACS_EXTRA2_PATH.read_text(encoding="utf-8"))
    ...
    for f in munis["features"]:
        ...
        muni_recs[tid] = {
            ...
            "extra": muni_acs_extra2.get(tid, {}),   # the new metrics
        }
