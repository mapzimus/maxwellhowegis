...
| [`/geopuesto/playground/`](https://maxwellhowegis.com/geopuesto/playground/) | Sibling research-grade spherical-geometry sandbox: Two-Point Mode with the four named equidistant points (M, −M, n, −n) and the Geomate pair IP, Polyhedra Suite (Platonic + Archimedean + Stella Octangula + Fibonacci sphere wrapped on the globe with a spin slider), Curves Suite (loxodromes, small-circles-at-distance-d) | Pure-JS spherical-math kernel, Leaflet |
