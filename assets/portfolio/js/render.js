...
(function () {
    ...
    function renderCard(item, opts) {
        ...
        if (item.thumb && opts.showThumb !== false) {
            var modernBase = item.tier === 'featured'
                ? window.V2.asset('images/projects/optimized/' + item.slug)
                : null;
            h += '<figure class="plate-figure">';
            if (modernBase) {
                h += '<picture><source srcset="' + esc(modernBase + '.avif') + '" type="image/avif">' +
                    '<source srcset="' + esc(modernBase + '.webp') + '" type="image/webp">';
            }
            h += '<img src="' + esc(window.V2.asset(item.thumb)) +
                '" alt="' + esc(item.title) + '" loading="lazy" decoding="async">';
            if (modernBase) h += '</picture>';
            h += '</figure>';
        }
