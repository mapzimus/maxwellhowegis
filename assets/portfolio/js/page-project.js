...
(function () {
    ...
    if (!p) {
        el.innerHTML = '<header class="page-head"><div class="kicker">404</div>' +
            ...
            '<a href="/work/">Back to all work</a>.</p></header>';
    ...
    h += '<div><p class="lede">' + esc(p.summary) + '</p>' +
        (p.role ? '<section class="case-section"><h2>My role</h2><p>' + esc(p.role) + '</p></section>' : '') +
        (p.description ? '<section class="case-section"><h2>Approach</h2><div class="detail-desc"><p>' + esc(p.description) + '</p></div></section>' : '') +
        (p.outcome ? '<section class="case-section"><h2>Result</h2><p>' + esc(p.outcome) + '</p></section>' : '') +
