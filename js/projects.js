...
const projects = [
    ...
    // ─── Current / Active ─────────────────────────────────────────────────────
    {
        ...
        category: "Spatial Analysis",
        type: "analysis",
        tags: ["R", "sf", "terra", "DEM / Hillshade", "Leaflet", "OpenStreetMap", "USGS WBD", "Census TIGER", "US Census 1920", "USGS historical topo"],
        summary: "A reproducible, multi-layer R GIS study of the Quabbin Reservoir and the four Swift River Valley towns (Dana, Enfield, Greenwich, Prescott) disincorporated in 1938 to supply metropolitan Boston. Layers include terrain, the reservoir, watershed, modern boundaries, decennial census population, a reservoir-filling animation, a valley cross-section, and the aqueduct to Boston — rendered to thirteen figures plus an interactive Leaflet map by a single R pipeline, all from open data.",
        description: "A multi-layer GIS study of how the Quabbin Reservoir was sited and what it replaced, built in R as a reproducible pipeline and shipped with its full analysis code. Between 1938 and 1946 Massachusetts dammed and flooded the Swift River Valley, disincorporating four towns — Dana, Enfield, Greenwich, and Prescott — and relocating about 2,500 residents to supply metropolitan Boston, roughly 100 km east. The pipeline pulls a DEM from AWS Terrain Tiles (elevatr), reprojects it to Massachusetts State Plane and hillshades it with terra; derives the reservoir from its 530-foot full-pool surface; adds USGS Watershed Boundary Dataset units for drainage context and US Census TIGER municipalities showing how the former town land is now divided among the surrounding towns. It also assembles decennial US Census counts (1900–1920) for all four towns; an animation of the reservoir filling in equal-area stages to the 1946 full pool; a west-east valley cross-section; a documented 'by the numbers' summary; a 3D terrain view; and the aqueduct route carrying the water east to Boston. These come together in an interactive Leaflet map with an adjustable pool level, the 1893 USGS pre-reservoir topographic quadrangle as a fade overlay, per-town census popups, and the aqueduct route. Every layer comes from open data, each network fetch falls back to a documented default if a service is unavailable, and a single `Rscript run_all.R` regenerates the figures, the animation, and the GeoJSON the map consumes.",
